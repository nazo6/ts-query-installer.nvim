...
local M = function(filetype)
  ...
  if source == nil then
    vim.notify("No query source found for filetype " .. filetype)
    return
  end
  if vim.fn.isdirectory(vim.fs.joinpath(utils.queries_dir, filetype)) ~= 0 then
    vim.notify("Query already installed for filetype " .. filetype)
    return
  end

  vim.notify("Installing query for filetype " .. filetype)
  ...
  vim.loop.spawn("git", {
    ...
    vim.schedule(function()
      if code ~= 0 then
        vim.notify("Failed to install query for filetype " .. filetype)
        return
      end

      local query_source = vim.fs.joinpath(repo_clone_path, source.path)
      ...
      vim.notify("Installed query for filetype " .. filetype)
    end)
  end)
end
